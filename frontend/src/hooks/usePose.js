 ...
 */
export function usePose(videoRef, canvasRef, onResults, active, personLocked, onLockData, lockTarget, isUpload = false) {
    const poseRef = useRef(null);
    const cameraRef = useRef(null);
    const frameLoopRef = useRef(null);

    // Mirror mutable props into refs so the pose callback always reads the latest
    // value without needing to restart the pose engine on every state change.
    const personLockedRef = useRef(personLocked);
    const lockTargetRef = useRef(lockTarget);
    useEffect(() => { personLockedRef.current = personLocked; }, [personLocked]);
    useEffect(() => { lockTargetRef.current = lockTarget; }, [lockTarget]);
    ...
    useEffect(() => {
        ...
        const initPose = () => {
            ...
            pose.onResults((results) => {
                // Read lock state from refs — always current, no stale closure
                const locked = personLockedRef.current;
                const target = lockTargetRef.current;

                if (!results.poseLandmarks) {
                    drawResults(results, true, locked);
                ...
                if (locked && target) {
                    ...
                    const dist = Math.sqrt(
                        Math.pow(cx - target.cx, 2) +
                        Math.pow(cy - target.cy, 2)
                    );
            ...
            if (isUpload) {
                ...
                const frameLoop = async (now, metadata) => {
                    if (!active) return;

                    if (video.readyState >= 2 && !video.paused && !video.ended) {
                        try {
                            if (video.videoWidth > 0 && !pendingFrames) {
                                pendingFrames = true;
                    ...
                    if (isPlaying) {
                        if (video.requestVideoFrameCallback) {
                            frameLoopRef.current = video.requestVideoFrameCallback(frameLoop);
                        } else {
                ...
                if (!video.paused && video.readyState >= 2) {
                    ...
                    // Send a single frame if paused but ready (e.g., loaded first frame)
                    pose.send({ image: video }).catch(e => console.error(e));
                }
            } else {
                // Live Camera mode
        ...
        // Give React a moment to apply the srcObject or src changes before init
